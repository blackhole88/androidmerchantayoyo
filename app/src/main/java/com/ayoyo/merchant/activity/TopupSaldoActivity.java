...
import com.ayoyo.merchant.json.fcm.FCMMessage;
import com.ayoyo.merchant.json.linkaja.EnableEddResponse;
import com.ayoyo.merchant.models.Notif;
...
import com.ayoyo.merchant.utils.api.FCMHelper;
...
public class TopupSaldoActivity extends AppCompatActivity {
    ...
    TextView textnotif;
    String disableback;
    LinearLayout banktransfer, creditcard, paypal,payumoney, linkaja;
        ...
        linkaja.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!nominal.getText().toString().isEmpty()) {
                    String idUser =  BaseApp.getInstance(TopupSaldoActivity.this).getLoginUser().getId();
                    IsiSaldoRequest request = new IsiSaldoRequest();
                    request.nominal = nominal.getText().toString().replace(".","").replace(",","");
                    request.id_user = idUser;
                    topUpViaLinkaja(request);
                } else {
...
//        }
    ...
    private void topUpViaLinkaja(IsiSaldoRequest request){
        PaymentService service = ServiceGenerator.createService(PaymentService.class);
        service.isiSaldo(request).enqueue(new Callback<EnableEddResponse>() {
            @Override
...
//                showLoading(false);
                if(response.body()!=null) {
                    if (response.body().message.equals("failed") && response.body().data.contains("edd")) {
                        ...
                        Intent intent = new Intent(TopupSaldoActivity.this, WebcheckoutActivity.class);
